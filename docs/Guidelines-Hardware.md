...
- [Schematic drawing](#schematics) (eg `*.pdf`)
- [PCB Design](#design)  (eg `*.brd` for OrCAD or `*.kicad_pcb` for KiCad)
- Gerber files (eg `*.art` for OrCAD or `*.gbr` for KiCad)
- drill file (eg `*.drl` for OrCAD and KiCad)
- Bill of Materials (eg `*.csv` or `*.xlsx`)
- Invoice or price estimate (eg `*.pdf` or text)
- Design this was based on

While the above files should be available for each hardware revision, some of the files below are nice-to-have, but not crucial. Others, like the manufacturers' feedback depend on the assembler and can take different forms (eg emails, screenshots, update files) and follow different schedules. Others, like photos and renderings, are purely for improving the documentation.

- placement file (eg `*.txt`)
- feedback from manufacturer
- [Schematics Design](#schematics) (eg `*.sch` for Cadence and KiCad)
...
The focus of the repositories is on the production of Modular LED Displays and, therefore, the PCB design. Yet, to understand the PCB design, it is necessary to track changes in the schematics. Therefore some visualization of the schematic drawing should be provided every time something has changed. In the past, these schematics were shown documents such as `*.pdf` files, but other images like `*.png` files can be OK, too. Some type of schematic drawing or a reference to the schematic previously used is required.

Since these files are generated from schematic design files, it can be helpful to track the schematic designs in the repositories as well. Both tools, OrCAD and (old versions of) KiCad use the file extension `*.sch` for the schematic design files, while recent versions of KiCad switched to `*.kicad_sch`. Some of the designs can consist of several schematic files or require additional library files that contain details for used electrical components. Please provide all files necessary for opening the schematic. As a user of your toolset you probably know best which files are required. 

The handover between the schematic design and the PCB design is, among others, a netlist file. Since the netlist can be generated from a valid schematic design file, it is not necessary to keep track of these files in the repositories.
...
For the two toolchains that have been used in the past, the PCB design file for OrCAD uses `*.brd` as a file extension while KiCad uses `*.kicad_pcb` at this stage. For example, the G4 panel driver starting with version 1 has been developed in OrCAD while the panel communication board is being developed in KiCAD. Consequently, the [repository for the driver](https://github.com/floesche/Panel-G4-Hardware) is used to track the latest design iteration in the OrCAD file format as a `*.brd` file. Similarly, the [repository for the comm board](https://github.com/floesche/panels_g4_hardware/tree/master/atmega328/four_panel/20mm_matrix/ver3/comm/) contains the latest design iteration in a KiCad file format which uses the extension `*.kicad_pcb`.

Some designs might require additional files, please make sure to share all files that are necessary to open the PCB design file. For example, footprints of additional components can be in a `*.kicad_mod` file.

## Fabrication files

"Fabrication files" is a set of files sufficient to produce the PCBs. The files are generated by the EDA tools from the PCB design files. Although file types are in general defined by industry standards, there are small differences between manufacturers and tools. OrCAD and KiCad support different settings for each of the generated files. It would be tedious to explain the different options, instead here we list some best practices that generate something similar to a lowest common denominator and that have proven to work so far. We hope to improve these descriptions over time.
...
Both tools generate one file per layer, for example copper layers, the solder paste layers on front and back, and the silkscreen. Sometimes an additional file is used to describe the edge cuts and adhesive layers can be used for some designs. For a four layer PCB this means that up to eleven gerber files are produced (4×copper, front + back silkscreen, front + back solder paste, front + back adhesive and edge cuts).
...
Emphasizing all features of the file format would be tedious, but there are a few settings that seem to work better than others. Specifically, many tools don't support the relatively new "repeat code" which makes the file shorter but computation on the tools more complex. For example, the command `R50X004` would repeatedly drill a hole 50 times along the X-axis with 4mm distance. Functionally this is the same as specifying all 50 locations. On the other hand, most CAM drills support the automatic selection of the correct drill, so they can be specified in the file header as opposed to a description in an external text.

#### OrCAD

In summary, for OrCAD the following settings have worked well. Note that *Auto tool select*{:.gui-txt} is selected and *Repeat codes*{:.gui-txt} is not, which is different from the default. Also, the drilling should be done by layer pair, which defines the starting and end layer for the drill, instead of generating a separate drill file for each individual layer. 
...
The Bill of Materials (BOM) is a spreadsheet that matches the component names from design with actual physical components. Similarly to the drill files there is no standard, but there are a number of recommendations and best practices. In general `*.csv` and `*.xlsx` files are accepted and we use both across the repositories.

Arguably the most important feature of a BOM are descriptive header: the manufacturer will open the file and then try to match columns to the expected fields with as little manual intervention and margin for error as possible.
...
Components are considered to be incompatible between generations, hence the generation acts as the "MAJOR" version.
