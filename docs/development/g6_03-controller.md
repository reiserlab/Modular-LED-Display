...
Reflashes a single panel from the firmware image on SD; see § Panel firmware update (ISP) for the full per-panel workflow. The image must already exist at `/firmware/panel.bin` (upload via `set-firmware-file (0xE0)`). Requires ALL_OFF. Blocks while the update runs; the `ISP_COMMIT` receipt and the post-reboot liveness are polled (ceilings 15 s and 12 s, see § Bench-tunable parameters), so the command returns as soon as the panel answers rather than after fixed worst-case waits. Typical total is ~3.2 s per panel (stream ~0.6 s + verify ~50 ms + commit receipt ~1.8 s + boot ~0.8 s; fleet-validated at the shipped 25 MHz phase-A clock and 500 us page pacing). (Relocated from `0x41`, which collided with the G4 `startLog` opcode.)
...
**Response (success):** `[len, 0x00, 0xC8, ASCII_msg]`, e.g. `panel N flashed via OTA (<bytes> bytes) in <t> s (stream <ms> + verify <ms> + commit <ms> + boot <ms> ms, <pages> pages)` (N is the 1-based number; the per-phase timing summary doubles as instrumentation for the bench sweeps).
...
- **ISP SPI clocks (split):** `IspController::kIspClockAMhz` = 25 MHz clocks phase A (the COPI command; no CIPO constraint, same bus as 25 MHz display streaming; fleet-validated, with 10 and 16 MHz also passing on the way up) and `kIspClockBMhz` = 2 MHz clocks phase B (the CIPO readback; the wired-OR return path slips ~1 bit per ~100 ns, so keep it low).
- **Inter-transaction pacing (split):** a command clocked before the panel is back in its receive loop is missed, and the following phase-B zeros then parse as a bad-header command (PE04). The slow one-shot commands (ENTER/VERIFY_STAGED/COMMIT) use `kCmdGapUs` = 2000 us; VERIFY_STAGED is built instantly after the last WRITE_PAGE reply and misses at ~1-in-20 with a 1000 us gap, so do not shorten it. The WRITE_PAGE loop uses `kPageGapUs`/`kPageReplyWaitUs` = 500/500 us (fleet-validated; page building adds its own SD-read gap on top). Re-sweep on the bench before going lower.
- **Commit / reboot polling:** the `ISP_COMMIT` receipt is polled every `kCommitPollMs` (250 ms) with a `kCommitTimeoutMs` ceiling (15 s; covers the LittleFS write plus a first-use **format**). Post-reboot liveness is polled with full COMM_CHECK frames every `kAlivePollMs` (250 ms) up to `kAliveTimeoutMs` (12 s; reboot + OTA copy + boot). COMM_CHECK is used deliberately: it is exempt from the panel's boot-indicator retire filter, so polling leaves the post-flash smiley in place, and bare zero-read transactions would parse as bad-header commands on a live panel. Size the ceilings up for multi-hundred-KB images.
