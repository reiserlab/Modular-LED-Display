...
4. **Board-id mechanism for firmware to detect v0.2 vs v0.3.** Build-time `#define` (separate firmware binaries — simplest)? Runtime ID via spare GPIO (e.g. v0.3's GP46 jumpered to GND on the panel; firmware reads at boot)? Magic bytes on PSRAM populated at factory? Decide before unified firmware ships.
...
- **2026-05-02** — v0.2 EINT (GP45), PSRAM CS (GP0), and SPI peripheral (SPI0) resolved by reconciling against `PANEL_V021_V031_HW_SUMMARY.md`; v0.3 MISO corrected from GP44 → GP43 (R29 termination is on the SPI1 MISO output, GP43, not GP44 spare); XIP_CS1n confirmed on GP47 (this commit).
