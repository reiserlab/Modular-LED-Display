...
- **ISP namespace evolution.** ISP opcodes live in the v1 namespace, so every future v2/v3/v4 panel firmware must keep honoring them; a dedicated ISP protocol with an explicit `BOOT_TO_ISP` transition would isolate the flash-write opcodes and version independently — kept in v1 for now (see [`g6_01-panel-protocol.md`](g6_01-panel-protocol.md) § In-System Programming → Open TBDs).
