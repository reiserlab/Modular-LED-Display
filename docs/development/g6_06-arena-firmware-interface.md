...
- **Production arena hardware:** [`reiserlab/LED-Display_G6_Hardware_Arena`](https://github.com/reiserlab/LED-Display_G6_Hardware_Arena) (in this clone as the `Generation 6/Arena` submodule, pinned at `a9ab466e`). Key contents:
...
| **Experiment I/O — Analog Output** | AOUT 0–5 V | — (I2C-driven) | — | DAC out → BNC J27 | **No direct Teensy pin.** Generated by an MCP4725 12-bit I²C DAC; firmware writes via I²C on D18/D19 (SDA/SCL, the default `Wire`). Update rate bounded by I²C clock (`Wire` at 400 kHz → ~6 kSa/s) |
...
| **Unused / future** | spare / NC | 30, 31, 32, 38, 39, 42 | D38–D41, D16, D17, D20 | `Net-(U1-NN_..)` or `no_connect` | Available for future hardware revisions; firmware should not assert these |

### Operational gotchas firmware MUST know

These three are spec-relevant — they change how firmware MUST be written. (Lower-priority background facts — AOUT/AI implementation details, level-translator notes, unused-pin enumeration — are in § Background facts firmware should know at the bottom.)

1. **EINT routing jumper J30 selects between Teensy-mediated and direct-to-panels external trigger.** When shorted, BNC J4 drives `TNY.EINT` directly through R216 (1 kΩ series) — Teensy can sense the line on D35 but is bypassed for the actual panel trigger; firmware-emitted D33 (`TNY.EINT`) still has effect via R25 33Ω, but the J4 BNC dominates if both are driven. When open, the Teensy is solely in the loop: firmware reads D35 (input), processes (timing reshape, gating decisions), and drives D33 (output) to the panels. **The jumper position is not firmware-detectable** — it must be a deployment-time config, documented and physically verified per arena. Note: BNC J3 silk-labeled "External Interrupt" is a *different* path (Teensy D37, level-translated, no jumper bypass) — the actual panel-trigger BNC is J4 despite its "0-5V Digital In/Out" silk label.
...
- **Panel brick-recovery:** each panel exposes its RP2350 BOOTSEL pin and a USB connector on the panel header for out-of-band reflash if in-firmware ISP (see [`g6_01-panel-protocol.md`](g6_01-panel-protocol.md) § In-System Programming) gets corrupted mid-flash. No arena-side wiring required.
...
The 4 CS lines per column are delivered to the 4 panels in that column via the **panel-internal J3↔J5 connector pin shift** mechanism. Each panel hardwires `J3 pin 5 → MCU CS0`; J3 pins 2/3/4 (carrying CS3/CS2/CS1) route to J5 pins 3/4/5 (same signals, one-higher pin number), so when J5 plugs into the next panel's J3 in a 1:1 pin-aligned stack, the upper panel's `J3 pin 5 → CS0` receives the lower panel's CS1, and so on up the stack. Full spec — including J5 pin 2 = NC and EINT pass-through on J3 pin 1 ↔ J5 pin 1 — is in [`g6_02-led-mapping.md`](g6_02-led-mapping.md) § Connectors (canonical, KiCad-verified against panel v0p3 production). Each column buffer drives 4 distinct CS lines, one per stack slot; no per-panel jumper is needed.
...
External-interrupt routing is also present per panel column (panel-internal `PAN.EINT_P1..P10`, all driven by **Teensy pin 25 / `D33` / `TNY.EINT`**, via R25 33 Ω series — see Pin assignments table at top of file for the canonical reference). This is the panel-side interrupt, distinct from the experimenter EINT BNC.
...
2. **Top-board design (J26 use).** Not yet published.
3. **2-channel AO future.** Would require arena re-spin.
...
- [`Generation 6/Arena/docs/arena.md`](../../Generation%206/Arena/docs/arena.md) — authoritative arena hardware reference (in submodule)
...
- [`g6_05-host-software.md`](g6_05-host-software.md) — host-side concerns; arena geometry must be supplied from host since controller is geometry-ignorant
